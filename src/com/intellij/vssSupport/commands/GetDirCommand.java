...
public class GetDirCommand extends VssCommandAbstract
{
  ...
  public void execute()
  {
    ...
    for( String file : listener.filesAdded )
      updatedFiles.getGroupById( FileGroup.LOCALLY_ADDED_ID ).add(file, vcsKey, null);

    for( String file : listener.filesChanged )
      updatedFiles.getGroupById( FileGroup.UPDATED_ID ).add(file, vcsKey, null);

    for( String file : listener.filesSkipped )
      updatedFiles.getGroupById( FileGroup.SKIPPED_ID ).add(file, vcsKey, null);
